...
function bindEvents() {
  ...
  refs.playSharedBtn?.addEventListener("click", async () => {
    ...
    if (!window.supcontentPlayer) { toast("Lecteur non disponible.", "Info"); return; }
    ...
    if (source.includes("youtube")) {
      window.supcontentPlayer.playYouTube({ url: String(found?.external_urls?.spotify || url), title: String(found?.name || title), subtitle: String(found?.artists?.[0]?.name || artist), cover: String(found?.album?.images?.[0]?.url || ""), mode: "audio" });
    } else {
      window.supcontentPlayer.playMedia({ url, title: String(found?.name || title), subtitle: String(found?.artists?.[0]?.name || artist), cover: String(found?.album?.images?.[0]?.url || ""), mode: "audio" });
    }
